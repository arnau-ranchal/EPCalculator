...
double E_0_co(double r, double rho, double &grad_rho, double &E0) {
    ...
    // Detect degenerate channel (all symbols indistinguishable)
    // This happens at SNR≈0 where D_mat rows are nearly identical
    ...
    // mp = sum_j [ (sum_i Q_i * pig1[i,j]) * qg2rho[j] * logqg2[j] ]
    //    - s * sum_j [ (sum_i Q_i * pig1[i,j] * D[i,j]) * qg2rho[j] ]
    
    std::vector<double> mp_positive_terms;
    std::vector<double> mp_negative_terms;

    for (int j = 0; j < cols; j++) {
        // Base: sum_i Q_i * pig1[i,j]
        Eigen::VectorXd inner_log_terms = Q_mat.array().log() + log_pig1_mat.col(j).array();
        double log_inner_sum = log_sum_exp(inner_log_terms);
        double log_base = log_inner_sum + rho * logqg2(j);

        // Term 1: (sum_i Q_i * pig1[i,j]) * qg2rho[j] * logqg2[j]
        if (logqg2(j) != 0) {
        ...
        // FIX: Only include terms where D[i,j] > 0 (to avoid log(0))
        std::vector<double> term2_inner_log_terms;
        for (int i = 0; i < sizeX; i++) {
            double D_val = D_mat(i, j);
            if (D_val > 0) {  // Only include non-zero D values
                double log_term = std::log(Q_mat(i)) + std::log(PI_mat(i, j))
                                + rho_factor * D_val + std::log(D_val);
                term2_inner_log_terms.push_back(log_term);
            }
        }
        
        if (!term2_inner_log_terms.empty()) {
            // Convert to Eigen for log_sum_exp
            Eigen::VectorXd term2_log_vec(term2_inner_log_terms.size());
            for (size_t k = 0; k < term2_inner_log_terms.size(); k++) {
                term2_log_vec(k) = term2_inner_log_terms[k];
            }
            double log_term2_inner = log_sum_exp(term2_log_vec);
            double log_term2 = std::log(s) + log_term2_inner + rho * logqg2(j);
            mp_positive_terms.push_back(log_term2);
        }
    }
    ...
    if (max_term < 690 && log_m < 690) {
        ...
        E0 = -log2_m + log2_PI;
        grad_rho = 0.0;  // Gradient unreliable in extreme case

        if (!std::isfinite(E0)) {
            std::cerr << "ERROR: Non-finite E0\n";
            E0 = -1.0;
            return -1.0;
        }
